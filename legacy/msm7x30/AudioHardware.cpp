...
static const uint32_t SND_DEVICE_XLOUD_SPEAKER = 20;
...
static const uint32_t DEVICE_XLOUD_SPEAKER_RX = 18;
static const uint32_t DEVICE_COUNT = DEVICE_XLOUD_SPEAKER_RX +1;
    ...
    mVoipFd(-1), mVoipInActive(false), mVoipOutActive(false), mDirectOutput(0)
{
        ...
        for(i = 0; i < dev_cnt;i++) {
            ...
            else if(strcmp((char*)name[i],"xloud_speaker_mono_rx") == 0)
                index = DEVICE_XLOUD_SPEAKER_RX;
            else
                continue;
            ALOGV("index = %d",index);
                                   ...
                                   bool ear_mute, bool mic_mute)
{
    ...
    if(device == SND_DEVICE_HANDSET) {
        ...
        new_rx_device = DEVICE_XLOUD_SPEAKER_RX;
        ...
        new_rx_device = DEVICE_XLOUD_SPEAKER_RX; //cur_rx;
